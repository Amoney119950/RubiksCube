...
public class ReadCube : MonoBehaviour
{
    ...
    private int layerMask = 1 << 8; // this layerMask is for the faces layer only
    CubeState cubeState;
    CubeMap cubeMap;
    ...
    public void ReadState()
    {
        ...
        this.cubeMap = FindObjectOfType<CubeMap>();

        // set the state of each position in the list of sides so we know
        // what color is in what position
        this.cubeState.up = ReadFace(upRays, tUp);
        this.cubeState.down = ReadFace(downRays, tDown);
        this.cubeState.left = ReadFace(leftRays, tLeft);
        this.cubeState.right = ReadFace(rightRays, tRight);
        this.cubeState.front = ReadFace(frontRays, tFront);
        this.cubeState.back = ReadFace(backRays, tBack);
    ...
    {
        // populate the ray lists with raycasts eminating from the trasform, angled towards the cube.
